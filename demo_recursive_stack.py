...
if args.source_EV == 0:
	target_EV_fold = [-3, -2, -1, 1, 2, 3]
elif args.source_EV == -1:
	target_EV_fold = [-3, -2, 0, 1, 2, 3]
elif args.source_EV == -2:
	target_EV_fold = [-3, -1, 0, 1, 2, 3]
elif args.source_EV == -3:
	target_EV_fold = [-2, -1, 0, 1, 2, 3]
...
for process_scene in scene_fold:
	...
	with torch.no_grad():
		...
		for target_EV in target_EV_fold: 
			# Processing source image into target EV
			print("Processing ", process_scene, " EV", str(args.source_EV), " to EV", str(target_EV))
			EV_step = target_EV - args.source_EV
			...
			output_path = save_path + process_scene + "/EV" + str(target_EV) + ".png"
