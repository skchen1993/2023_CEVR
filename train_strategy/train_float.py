...
parser.add_argument('--F_coff', type=int, default=0.1)
...
# record
parser.add_argument('--writer_path', type=str, default='runs/exp')
...
for epoch in tqdm(range(args.num_epoch)):
    ...
    for data in train_loader:
        ...
        loss_l1 = criterion(out, gt)

        # Float loss v1 => 直接/2 ev
        out_temp = model(img, step/2, ori)
        out_float = model(out_temp, step/2, step/2)
        loss_float = criterion(out_float, gt)

        loss = loss_l1  + loss_float * args.F_coff
        ...
        train_loss_float += loss_float.item()
        train_psnr += psnr

    train_loss /= len(trainset)
    train_loss_float /= len(trainset)
    ...
    if args.wandb == True:
        wandb.log({"train_loss": train_loss, "test_loss": test_loss, "train_psnr": train_psnr, "test_psnr": test_psnr, "train_loss_float": train_loss_float}) 
