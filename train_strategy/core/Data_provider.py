...
class VDS_dataset(Dataset):
    def __init__(self, root:str, training:bool, img_num=7, img_size=(256,256), center=0, transform=None) -> None:
        super().__init__()
        self.root=root
        ...
        self.data_stack = os.listdir(self.root)
        if transform:
    ...
    def __getitem__(self, index):
        stack_ind = index // self.img_num
        set_ind = self.center
        img_ind = (index % self.img_num) - (self.img_num//2)
...
class VDS_full_dataset(Dataset):
    def __init__(self, root:str, training:bool, img_num=7, img_set=7, img_size=(256,256), transform=None) -> None:
        super().__init__()
        self.root=root
        self.img_num = img_num
    ...
    def __len__(self) -> int:
        return len(self.data_stack) * self.img_num * self.img_set

    def __getitem__(self, index):
        num_pairs = self.img_num * self.img_set
        stack_ind = index // num_pairs
        set_ind = (index % num_pairs) // self.img_num - (self.img_set//2)
        img_ind = (index % self.img_num) - (self.img_num//2)
        ...
        gt_img_path = self.stack_img(self.data_stack[stack_ind], img_ind)
        gt_img = Image.open(gt_img_path)
        if self.transform:
            gt_img = self.transform(gt_img)
        ...
        step = torch.tensor([set_ind + img_ind], dtype=torch.float32)
        origin = torch.tensor([set_ind], dtype=torch.float32)
        #print(stack_ind, set_ind, img_ind)
        
        return img, gt_img, step, origin

class VDS_eval_dataset(Dataset):
    def __init__(self, root:str, training:bool, img_num=7, img_size=(256,256), center=0, transform=None) -> None:
        super().__init__()
        self.root=root
        self.img_num = img_num -1
        self.center = center
...
class VDS_half_full_dataset(Dataset):
    def __init__(self, root:str, training:bool, increase:bool, img_num=7, img_set=4, img_size=(256,256), transform=None) -> None:
        super().__init__()
        self.root=root
        self.img_num = img_num //2 +1
        self.img_set = img_set
        if increase:
            self.center = 0
        else:
    ...
    def __getitem__(self, index):
        num_pairs = self.img_num * self.img_set
        stack_ind = index // num_pairs
        set_ind = (index % num_pairs) // self.img_num + self.center
        img_ind = (index % self.img_num) + self.center
        ...
        step = torch.tensor([img_ind - set_ind], dtype=torch.float32)
        origin = torch.tensor([set_ind], dtype=torch.float32)
        #print(stack_ind, set_ind, img_ind, img_ind - set_ind)

        return img, gt_img, step, origin

class VDS_semi_dataset(Dataset):
    def __init__(self, root:str, training:bool, increase:bool, img_num=7, img_size=(256,256), center=0, transform=None) -> None:
        super().__init__()
        self.root=root
        self.img_num=(img_num // 2)
        self.center=center
    ...
    def __getitem__(self, index):
        stack_ind = index // self.img_num
        set_ind = self.center
        img_ind = (index % self.img_num) + self.base
...
class Eye_semi_dataset(Dataset):
    def __init__(self, root:str, increase:bool, img_num=7, img_size=(256,256), center=0, transform=None) -> None:
        super().__init__()
        self.root=root
        self.img_num=(img_num // 2)
        self.center=center
        ...
        if increase:
            self.base = 1
            self.index = [0, 0.7, 1.3, 2, 2.7]
        else:
            self.base = -1 * self.img_num
            self.index = [0, -2.7, -2, -1.3, -0.7]
    ...
    def __getitem__(self, index):
        ...
        step = torch.tensor([self.index[set_ind + img_ind]], dtype=torch.float32)
        origin = torch.tensor([set_ind], dtype=torch.float32)
        #print(stack_ind, img_ind, set_ind)
...
def build_dataset(args):
    ...
    if args.set_name == 'base':
        dataset = VDS_dataset(args.data_root, training=True, img_num=args.img_num, img_size=img_size)
        test_set = VDS_dataset(args.data_root, training=False, img_num=args.img_num, img_size=img_size)

    if args.set_name == 'full':
        dataset = VDS_full_dataset(args.data_root, training=True, img_num=args.img_num, img_set=args.img_set, img_size=img_size)
        test_set = VDS_full_dataset(args.data_root, training=False, img_num=args.img_num, img_size=img_size)
    ...
    if args.set_name == 'half_full':
        dataset = VDS_half_full_dataset(args.data_root, training=True, increase=increase, img_num=args.img_num, img_set=args.img_set, img_size=img_size)
        test_set = VDS_half_full_dataset(args.data_root, training=False, increase=increase, img_num=args.img_num, img_size=img_size)
