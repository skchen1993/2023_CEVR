...
parser.add_argument('--C_coff', type=int, default=0.1)
...
for epoch in tqdm(range(args.num_epoch)):
    ...
    for data in train_loader:
        ...
        out_cycle = model(out_temp, step_back, step)
        loss_cycle = criterion(out_cycle, img)


        loss = loss_l1  + loss_cycle * args.C_coff
        ...
        train_loss_cycle += loss_cycle.item()
        train_psnr += psnr

    train_loss /= len(trainset)
    train_loss_cycle /= len(trainset)
    ...
    if args.wandb == True:
        wandb.log({"train_loss": train_loss, "test_loss": test_loss, "train_psnr": train_psnr, "test_psnr": test_psnr, "train_loss_cycle": train_loss_cycle}) 
