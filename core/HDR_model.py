...
class RGB_layer(nn.Module):
    def __init__(self, in_dim, hid_lay, out_dim):
        super().__init__()
        # 64, [32, 16], 3
        self.conv1 = conv3x3(in_channels=in_dim, out_channels=hid_lay[0])
        self.conv2 = conv3x3(in_channels=hid_lay[0], out_channels=hid_lay[1])
        self.conv3 = conv3x3(in_channels=hid_lay[1], out_channels=out_dim)
...
class adj_layer(nn.Module):
    def __init__(self, in_dim, hid_lay, out_dim):
        ...
        self.conv1 = conv3x3(in_channels=in_dim, out_channels=hid_lay[0])
        self.conv2 = conv3x3(in_channels=hid_lay[0], out_channels=hid_lay[1])
        self.conv3 = conv3x3(in_channels=hid_lay[1], out_channels=hid_lay[2])
        self.conv4 = conv3x3(in_channels=hid_lay[2], out_channels=out_dim)
        self.act = torch.nn.ReLU()
...
class HDR_UNet_BN_Res_Add(nn.Module):
    def __init__(self, mlp_layer=2, pretrain=True, Decoder=Upsample_MLP, activation='relu'):
        super().__init__()
        ...
        self.neck = Bottel_neck(256, 256, 32, 32, mlp_layer, self.downsample, act=self.act)

        self.decoder3 = Decoder(256, 256, 256, 64, 64, mlp_layer, act=self.act)
        self.decoder4 = Decoder(256, 128, 128, 128, 128, mlp_layer, act=self.act)
        self.decoder5 = Decoder(128, 64, 64, 256, 256, mlp_layer, act=self.act)
...
class HDR_UNet_BN_Res_Affine_pad(nn.Module):
    def __init__(self, mlp_layer=2, pretrain=True, Decoder=Upsample_MLP, activation='relu', EV_info=1, init=False):
        ...
        if self.EV_info == 2:
            ...
            self.conv_rgb = nn.Conv2d(65, 3, kernel_size=(1, 1), stride=(1, 1))
            self.conv_mul = nn.Conv2d(65, 3, kernel_size=(1, 1), stride=(1, 1))

        elif self.EV_info == 3:
            self.conv_rgb = nn.Conv2d(80, 3, kernel_size=(1, 1), stride=(1, 1))
            self.conv_mul = nn.Conv2d(80, 3, kernel_size=(1, 1), stride=(1, 1))
...
class HDR_UNet_BN_Res_Affine3_pad(nn.Module):
    def __init__(self, mlp_layer=2, pretrain=True, Decoder=Upsample_MLP, activation='relu', EV_info=1, init=False):
        super().__init__()
        self.act = get_activation(activation)
        self.EV_info = EV_info
        ...
        if self.EV_info == 2:
            self.conv_adj = nn.Conv2d(66, 2, kernel_size=(1, 1), stride=(1, 1))

        elif self.EV_info == 1:
            self.conv_adj = nn.Conv2d(65, 2, kernel_size=(1, 1), stride=(1, 1))

        elif self.EV_info == 3:
            self.conv_adj = nn.Conv2d(80, 2, kernel_size=(1, 1), stride=(1, 1))
    ...
    def forward(self, x, step, origin):
        ...
        adj_map = self.conv_adj(feature)
        #adjust map
        alpha = (adj_map[:, 0, :, :]).unsqueeze(1)
        beta = (adj_map[:, 1, :, :]).unsqueeze(1)

        return beta + in_img * alpha 
...
def build_network(args):
    ...
    implemented_networks = ('add', 'affine', 'affine_pad', 'affine2_pad', 'affine3_pad', 'affine2_pad_Noaff')
    ...
    if net_name == 'add':
        net = HDR_UNet_BN_Res_Add(pretrain=pretraining, mlp_layer=args.mlp_num, Decoder=decoder, activation=args.act)

    if net_name == 'affine':
        net = HDR_UNet_BN_Res_Affine(pretrain=pretraining, mlp_layer=args.mlp_num, Decoder=decoder, activation=args.act)

    if net_name == 'affine_pad':
        net = HDR_UNet_BN_Res_Affine_pad(pretrain=pretraining, mlp_layer=args.mlp_num, Decoder=decoder, activation=args.act, EV_info=args.EV_info, init=args.init_weight)
    ...
    if net_name == 'affine3_pad':
        net = HDR_UNet_BN_Res_Affine3_pad(pretrain=pretraining, mlp_layer=args.mlp_num, Decoder=decoder, activation=args.act, EV_info=args.EV_info, init=args.init_weight)
